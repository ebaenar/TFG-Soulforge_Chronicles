...
MenuHandlers.add(:pc_menu, :pokemon_storage, {
  "name"      => proc {
    next ($player.seen_storage_creator) ? _INTL("Almacén de {1}", pbGetStorageCreator) : _INTL("Almacén de alguien")
  },
  "order"     => 10,
  "effect"    => proc { |menu|
    pbMessage(_INTL("\\se[PC access]Has inicializado el almacén."))
    command = 0
    loop do
      command = pbShowCommandsWithHelp(nil,
         [_INTL("Organizar"),
          ...
          _INTL("Depositar"),
          _INTL("Salir")],
         [_INTL("Organizar monstruos."),
          _INTL("Sacar."),
          _INTL("Guardar."),
          _INTL("Atrás.")], -1, command)
      break if command < 0
      case command
      when 0   # Organize
        pbFadeOutIn {
      ...
      when 2   # Deposit
        ...
        if count <= 1
          pbMessage(_INTL("No puedes guardar tu último monstruo."))
          next
        end
        pbFadeOutIn {
          scene = PokemonStorageScene.new
...
def pbChangeLevel(pkmn, new_level, scene)
  ...
  if old_level > new_level
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1} subió a nivel {2}!", pkmn.name, pkmn.level))
    else
      pbMessage(_INTL("{1} subió a nivel {2}!", pkmn.name, pkmn.level))
    end
    total_hp_diff        = pkmn.totalhp - old_total_hp
    attack_diff          = pkmn.attack - old_attack
    defense_diff         = pkmn.defense - old_defense
...
class PokemonEvolutionScene
  def pbEvolutionSuccess
    ...
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("\\se[]¡Felicidades! Tu {1} ha evolucionado a {2}!\\wt[80]",
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("Oh, no. No puedo comprarte {1}.", itemnameplural))
        next
      end
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(
          _INTL("¿Cuantos {1} quieres vender?", itemnameplural), item, qty
        )
      end
      if qty == 0
        @scene.pbHideMoney
      ...
      price *= qty
      if pbConfirm(_INTL("Puedo darte ${1}.\n¿Está bien?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("Has vendido {1} y has ganado ${2}.",
